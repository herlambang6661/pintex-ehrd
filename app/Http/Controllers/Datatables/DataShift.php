...
class DataShift extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = DB::table('shift')->orderBy('shift')->get();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = $btn . ' <a href="javascript:void(0)" data-toggle="tooltip" data-nama="' . $row->shift . '" data-id="' . $row->id . '" data-original-title="Delete" class="btn btn-outline-danger btn-sm btn-icon deleteSurat"><i class="fa-solid fa-fw fa-trash-can"></i></a>';
                    return $btn;
                })
