...
@section('content')
                        ...
                        <div class="col-6">
                            <div class="card card-xl border-success shadow rounded">
                                ...
                                <table style="width:100%; font-family: 'Trebuchet MS', Helvetica, sans-serif;"
                                    class="display table table-vcenter card-table table-sm table-bordered table-hover text-nowrap datatable-entitas"
                                    id="tblamaran">
                                    <thead>
                        ...
                        <div class="col-6">
                            <div class="card card-xl border-blue-lt shadow rounded">
                                <div class="card-header bg-primary text-white">
                                    <h5 class="modal-title"><i class="fa-solid fa-file-circle-plus"></i> Buat
                                        {{ $judul }}
                                    </h5>
                                </div>
                                <div class="card-body">
                                    <form id="formEntitas" name="formEntitas" method="post" action="javascript:void(0)">
                                        @csrf
                                        <div class="card-stamp card-stamp-lg">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control border border-dark" name="singkatan"
                                                id="singkatan" placeholder="Masukkan singkatan ">
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Nama</label>
                                            <input type="text" class="form-control border border-dark" name="nama"
                                                id="nama" placeholder="Masukkan Nama Surat">
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label">Alamat</label>
                                            <input type="text" class="form-control border border-dark" name="alamat"
                                                id="alamat" placeholder="Masukkan alamat Surat">
                                        </div>
                                        <div class="modal-footer">
                                            ...
                                            <button type="submit" id="submitEntitas" class="btn btn-primary ms-auto">
    ...
    {{-- Modal View --}}
    <div class="modal modal-blur fade" id="modal-view-entitas" tabindex="-1" role="dialog"
            ...
            <div class="modal-content">
                <form action="javascript:void(0)" method="post">
                    ...
                    <div class="modal-header bg-secondary text-white">
                        <h5 class="modal-title" id="exampleModalLabel">Detail Daftar Entitas</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="fetched-view-entitas"></div>
                    </div>
                    <div class="modal-footer">
    ...
    {{-- Modal edit --}}
    <div class="modal modal-blur fade" id="modal-edit-entitas" tabindex="-1" role="dialog"
        aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="javascript:void(0)" method="post">
    ...
    <script class="text/javascript">
        ...
        $(function() {
            ...
            var tableEntitas = $('.datatable-entitas').DataTable({
            ...
            --------------------------------------------==============================================================================================================================================================
            --------------------------------------------==============================================================================================================================================================*/
            if ($("#formEntitas").length > 0) {
                $("#formEntitas").validate({
                    rules: {
                        singkatan: {
                            required: true,
                        },
                    ...
                    submitHandler: function(form) {
                        ...
                        $("#submitEntitas").attr("disabled", true);
                        $.ajax({
                            url: "{{ url('storedataEntitas') }}",
                            type: "POST",
                            data: $('#formEntitas').serialize(),
                            beforeSend: function() {
                                Swal.fire({
                            ...
                            success: function(response) {
                                ...
                                $("#submitEntitas").attr("disabled", false);
                                tableEntitas.ajax.reload();
                                ...
                                document.getElementById("formEntitas").reset();
            ...
            $('#modal-view-entitas').on('show.bs.modal', function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ url('detail/entitas') }}',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        id: rowid,
                    },
                    success: function(data) {
                        $('.fetched-view-entitas').html(
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                $('#modal-edit-entitas .fetched-edit-entitas').html(`
        <div class="card-stamp card-stamp-lg">
            <div class="card-stamp-icon bg-primary">
                <i class="fa-solid fa-pen-to-square"></i>
            </div>
        ...
        <div class="mb-3">
            <label class="form-label">Singkatan</label>
            <input type="text" class="form-control border border-dark" name="singkatan" id="editsingkatan" value="${singkatan}">
        </div>
        <div class="mb-3">
            <label class="form-label">Nama</label>
            <input type="text" class="form-control border border-dark" name="nama" id="editnama" value="${nama}">
        </div>
        <div class="mb-3">
            <label class="form-label">Alamat</label>
            <input type="text" class="form-control border border-dark" name="alamat" id="editalamat" value="${alamat}">
        </div>
        <div class="modal-footer">
            <button type="button" id="submiteditentitas" class="btn btn-success" data-bs-dismiss="modal" data-id="${id}">
    ...
    `);
            ...
            $(document).on('click', '#submiteditentitas', function(e) {
                e.preventDefault();
                ...
                var formData = {
                    '_token': csrfToken,
                    'id': id,
                    'singkatan': $('#editsingkatan').val(),
                    'nama': $('#editnama').val(),
                    'alamat': $('#editalamat').val(),
                };

                $.ajax({
                    url: '/update/entitas', // Menggunakan URL endpoint tanpa menggunakan blade syntax
                    method: 'POST',
                    data: formData,
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            ...
            /*------------------------------------------
                ...
                --------------------------------------------
                --------------------------------------------*/
            $('body').on('click', '.deleteEntitas', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "DELETE",
                            url: "{{ route('getentitas.store') }}" + '/' + contract_id,
                            data: {
                                "_token": "{{ csrf_token() }}",
                            },
                            beforeSend: function() {
                            ...
                            success: function(data) {
                                $('.datatable-entitas').DataTable().ajax.reload();
                                const Toast = Swal.mixin({
                                    toast: true,
                                    position: "top-end",
                                    showConfirmButton: false,
